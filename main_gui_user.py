...
# Create an instance of UserManager to handle login
...
def show_main_window():
    ...
    def show_categories():
        ...
        for category in inventory.get_categories().values():
            output_text += f"ID: {category.id}, Category: {category.name}\n{'-'*40}\n"
        display_output(output_text)
    ...
    def show_products_by_category():
        def submit_category_id():
            try:
                ...
                if category_id in range(0, inventory.get_max_category_id() + 1):
                    ...
                    messagebox.showwarning("Invalid Category ID", "The entered category ID is not valid.")
        ...
        submit_button = tk.Button(new_category_window, text="Show Products by Category", command=submit_category_id)
    ...
    def add_product():
            ...
            try:
                ...
                if len(details) == 4:
                    ...
                    new_product = {"name": product_name, "price": float(price),
                                "quantity": int(quantity), "category": int(category_id)}
                    inventory.add_product(new_product)
                    messagebox.showinfo("Success", f"Product '{product_name}' added successfully.")
                    new_window.destroy()
                else:
                    messagebox.showwarning("Input Error", "Please provide product details in the format: name, price, quantity, category_id.")
        ...
        tk.Label(new_window, text="Enter product details (name, price, quantity, category_id):").pack(pady=10)
        entry_add_product = tk.Entry(new_window)
        entry_add_product.pack(pady=10)
        submit_button = tk.Button(new_window, text="Add Product", command=submit_product)
        submit_button.pack(pady=10)
    ...
    def add_category():
        ...
        submit_button = tk.Button(new_window, text="Add Category", command=submit_category)
        submit_button.pack(pady=10)
    ...
    def update_product():
        def submit_update():
            try:
                ...
                if not product_id or not new_value:
                    messagebox.showwarning("Input Error", "Please provide both product ID and new value.")
                    return

                if choice.get() == 1:  # Update product name
                    inventory.update_product_name(int(product_id), new_value)
                    messagebox.showinfo("Success", f"Product {product_id} name updated to '{new_value}'.")
                elif choice.get() == 2:  # Update product price
                    inventory.update_product_price(int(product_id), float(new_value))
                    messagebox.showinfo("Success", f"Product {product_id} price updated to {new_value}.")
                elif choice.get() == 3:  # Update product quantity
                    inventory.update_product_quantity(int(product_id), int(new_value))
                    messagebox.showinfo("Success", f"Product {product_id} quantity updated to {new_value}.")
                elif choice.get() == 4:  # Update category
                    inventory.update_category_name(int(product_id), new_value)
                    messagebox.showinfo("Success", f"Product {product_id} category updated to '{new_value}'.")
                else:
                    messagebox.showwarning("Input Error", "Please select a valid update option.")
        ...
        tk.Radiobutton(new_window, text="Product Name", variable=choice, value=1).pack()
        tk.Radiobutton(new_window, text="Product Price", variable=choice, value=2).pack()
        tk.Radiobutton(new_window, text="Product Quantity", variable=choice, value=3).pack()
        tk.Radiobutton(new_window, text="Category", variable=choice, value=4).pack()
        ...
        # Submit button
        submit_button = tk.Button(new_window, text="Submit Update", command=submit_update)
        submit_button.pack(pady=10)
    ...
    def remove_item():
            ...
            try:
                ...
                if choice == 1:  # Remove Product
                    ...
                    if category_id:
                        ...
                        messagebox.showinfo("Success", f"Category with ID {category_id} removed successfully.")
                    else:
                        messagebox.showwarning("Input Error", "Please enter a valid category ID.")
                else:
                    messagebox.showwarning("Choice Error", "Please select either Product or Category to remove.")
        ...
        tk.Radiobutton(new_window, text="Product", variable=var_choice, value=1).pack()
        tk.Radiobutton(new_window, text="Category", variable=var_choice, value=2).pack()

        # Entry field for ID input
        entry_id_label = tk.Label(new_window, text="Enter the ID of the item to remove:")
        entry_id_label.pack(pady=5)
        entry_id = tk.Entry(new_window)
        entry_id.pack(pady=5)

        # Submit button
        submit_button = tk.Button(new_window, text="Submit Removal", command=submit_removal)
        submit_button.pack(pady=10)

    def show_inventory_value_options():
        def show_total_value():
            total_value = inventory.get_total_inventory_value()
            output_text = f"\nTotal Inventory Value for All Products: ${total_value:.2f}\n"
            display_output(output_text)
            new_window.destroy()
        ...
        def show_value_by_category():
            def submit_category_id():
                try:
                    category_id = int(entry_category_id.get())
                    if category_id in range(0, inventory.get_max_category_id() + 1):
                        total_value = inventory.get_total_inventory_value_by_category(category_id)
                        output_text = f"\nTotal Inventory Value for Category ID {category_id}: ${total_value:.2f}\n"
                        display_output(output_text)
                        new_category_window.destroy()
                    else:
                        messagebox.showwarning("Invalid Input", "Category ID is not valid.")
                except ValueError as e:
                    messagebox.showerror("Error", f"Invalid input: {e}")
            ...
            submit_button = tk.Button(new_category_window, text="Get Value by Category", command=submit_category_id)
            submit_button.pack(pady=10)
        ...
        btn_total_value = tk.Button(new_window, text="Total Inventory Value (All Products)", command=show_total_value)
        btn_total_value.pack(pady=10)

        btn_value_by_category = tk.Button(new_window, text="Total Value by Category", command=show_value_by_category)
        btn_value_by_category.pack(pady=10)

    # Initialize the main window
    window.deiconify()  # Show the main window after login
    ...
    background_label.place(relwidth=1, relheight=1)  # Make sure it covers the full window
    ...
    btn_show_products = tk.Button(button_frame, text="Show Products", command=show_products, width=20)
    btn_show_products.pack(side=tk.LEFT, padx=5)

    btn_show_categories = tk.Button(button_frame, text="Show Categories", command=show_categories, width=20)
    btn_show_categories.pack(side=tk.LEFT, padx=5)

    # Add a new button for showing products by category
    btn_show_products_by_category = tk.Button(button_frame, text="Show Products by Category", command=show_products_by_category, width=30)
    btn_show_products_by_category.pack(side=tk.LEFT, padx=5)

    btn_search_product = tk.Button(button_frame, text="Search Product", command=search_product, width=20)
    btn_search_product.pack(side=tk.LEFT, padx=5)

    btn_add_product = tk.Button(button_frame, text="Add Product", command=add_product, width=20)
    btn_add_product.pack(side=tk.LEFT, padx=5)

    btn_add_category = tk.Button(button_frame, text="Add Category", command=add_category, width=20)
    btn_add_category.pack(side=tk.LEFT, padx=5)

    # Add the "Update Product" button to the main window
    btn_update_product = tk.Button(button_frame, text="Update Product", command=update_product, width=20)
    btn_update_product.pack(side=tk.LEFT, padx=5)

    # Add the "Remove Product/Category" button to the main window
    btn_remove_item = tk.Button(button_frame, text="Remove Product/Category", command=remove_item, width=20)
    btn_remove_item.pack(side=tk.LEFT, padx=5)

    # Add a new button for Total Inventory Value options
    btn_inventory_value_options = tk.Button(button_frame, text="Total Inventory Value", command=show_inventory_value_options, width=30)
    btn_inventory_value_options.pack(side=tk.LEFT, padx=5)

    # Create a label for output display
    output_label = tk.Label(window, text="Result:", fg="white", bg="black", font=("Arial", 18, "bold"))
    output_label.pack(pady=10)

    # Create a scrollable output box with increased width and height
    output_box = scrolledtext.ScrolledText(window, wrap=tk.WORD, width=100, height=35, font=("Arial", 12), bg="#cacaca", fg="black")
...
# Show login window when the application starts
show_login_window()
